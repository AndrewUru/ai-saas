      ...
      <div className="mx-auto max-w-7xl px-6 py-8 lg:px-8">
        ...
        <div className="mb-8 grid gap-4 sm:grid-cols-4">
           <div className="ui-card p-4 flex flex-col justify-between">
            <span className="text-[10px] uppercase tracking-wider text-[var(--foreground-muted)]">Agents Created</span>
          ...
          <div className="ui-card p-4 flex flex-col justify-between">
            <span className="text-[10px] uppercase tracking-wider text-[var(--foreground-muted)]">Default Limit</span>
            <span className="text-xl font-bold mt-1">{defaultMessagesLimit.toLocaleString("en-US")}</span>
          </div>
          ...
          <div className="ui-card p-4 flex flex-col justify-between sm:col-span-2 bg-gradient-to-br from-surface to-surface-strong/50">
             <div className="flex items-center justify-between">
                <div>
                   <span className="text-[10px] uppercase tracking-wider text-[var(--foreground-muted)]">Need help?</span>
                   <p className="text-sm font-medium mt-1">Check our detailed integration guides</p>
                </div>
                <Link href="/docs" className="ui-button ui-button--subtle text-xs">View Docs</Link>
             </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            {!agents?.length ? (
               <div className="ui-card flex flex-col items-center justify-center p-12 text-center border-dashed border-2 border-border/50 bg-transparent">
                  ...
                  <p className="text-[var(--foreground-muted)] max-w-sm mt-2">
                    Create your first agent to generate an API key and start automating your ecommerce support.
                  </p>
               </div>
            ) : (
              <ul className="space-y-4">
                {agents.map((agent) => {
                  const statusColor = agent.is_active ? "bg-accent" : "bg-[var(--foreground-muted)]";
                   const statusText = agent.is_active ? "Active" : "Paused";
                   const maskedKey = `${agent.api_key?.slice(0, 6) ?? "N/A"}...`;

                   return (
                     <li key={agent.id} className="ui-card group p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-all hover:border-accent/30 hover:shadow-[0_0_20px_rgba(52,211,153,0.05)]">
                        <div className="space-y-2">
                           <div className="flex items-center gap-3">
                              <span className={`h-2.5 w-2.5 rounded-full shadow-[0_0_8px_currentColor] ${statusColor}`} />
                           ...
                           <div className="flex items-center gap-4 text-xs text-[var(--foreground-muted)]">
                              <span className="flex items-center gap-1.5 bg-surface/50 rounded-md px-2 py-1">
                                 ...
                                 <span className="font-mono text-foreground/80">{maskedKey}</span>
                              </span>
                              <span>Limit: {agent.messages_limit?.toLocaleString() ?? "∞"}</span>
                           </div>
                        </div>
          ...
          <aside className="lg:col-span-4">
             <div className="sticky top-24 ui-card p-6 bg-surface-strong/30 backdrop-blur-xl border-accent/20">
                <div className="mb-6">
                   <span className="text-xs font-bold uppercase tracking-widest text-accent">New Agent</span>
                   <h2 className="text-xl font-bold text-foreground mt-2">Deploy new instance</h2>
                   <p className="text-sm text-[var(--foreground-muted)] mt-2">
                      Generate a new API key immediately. You can rename and reconfigure it anytime.
                   </p>
                </div>
                ...
                <form action={createAgent} className="space-y-4">
                   ...
                   <button type="submit" className="ui-button ui-button--primary w-full justify-center py-3 text-sm">
                      Generate Agent
                   </button>
                </form>
