export function Features() {
  const features = [
    ...
    },
    {
      ...
      icon: (
        <path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10zM7 12l3 3 7-7" />
      ),
    },
    {
    ...
    <section className="relative bg-[#030303] py-24 sm:py-32">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="max-w-3xl">
          <h2 className="text-base font-semibold leading-7 text-accent uppercase tracking-[0.2em]">
            Agency Infrastructure
          </h2>
          <p className="mt-2 text-4xl font-bold tracking-tight text-white sm:text-5xl">
            Everything you need to launch{" "}
            <span className="text-zinc-500">profitable agents.</span>
          </p>
        ...
        <div className="mt-20 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {features.map((feature) => (
            ...
            >
              ...
              <div className="pointer-events-none absolute -inset-px opacity-0 group-hover:opacity-100 transition duration-300 bg-[radial-gradient(600px_circle_at_var(--mouse-x)_var(--mouse-y),rgba(255,255,255,0.06),transparent_40%)]" />

              <div>
                <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-white/5 border border-white/10 text-white group-hover:text-accent group-hover:border-accent/30 transition-colors">
                  <svg
                    className="h-6 w-6"
                    fill="none"
                    viewBox="0 0 24 24"
                ...
                <h3 className="mt-6 text-lg font-semibold leading-7 text-white group-hover:text-accent transition-colors">
                  {feature.title}
                </h3>
                <p className="mt-3 text-sm leading-6 text-zinc-400 group-hover:text-zinc-300 transition-colors">
                  {feature.body}
                </p>
              </div>

              {/* Decoración sutil de esquina */}
              <div className="mt-6 flex items-center gap-x-2 text-xs font-medium text-zinc-500 group-hover:text-white transition-colors">
